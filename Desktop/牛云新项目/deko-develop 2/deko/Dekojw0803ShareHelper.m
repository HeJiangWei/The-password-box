...
@interface Dekojw0803ShareHelper () <MFMailComposeViewControllerDelegate>
@property (nonatomic) BOOL purchased;
@end

@implementation Dekojw0803ShareHelper
...
- (void)_saveImageToPhotos:(UIImage *)image
{
	...
	[library saveImageData:[self _imageDataFromImage:image] toAlbum:NSLocalizedString(@"444", @"Photo album title, do not localize") withCompletionBlock:^(NSError *error) {
		[self.delegate shareHelper:self savedImageWithError:error];
	}];
}
