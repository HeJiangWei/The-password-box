...
- (void)backup:(CDVjw0802InvokedUrlCommand*)command
{
    NSString* callbackId = command.callbackId;

    NSError* __autoreleasing error = nil;
    CDVjw0802PluginResult* result = nil;
        ...
        if ([info shouldBackup]) {
            ...
            if (callbackId) {
                if (error == nil) {
                    ...
                    result = [CDVjw0802PluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:message];
                    [self.commandDelegate sendPluginResult:result callbackId:callbackId];
                } else {
                    message = [NSString stringWithFormat:@"Error in CDVLocalStorage (%@) backup: %@", info.label, [error localizedDescription]];
                    NSLog(@"%@", message);

                    result = [CDVjw0802PluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:message];
                    [self.commandDelegate sendPluginResult:result callbackId:callbackId];
                }
            }
        }
...
- (void)restore:(CDVjw0802InvokedUrlCommand*)command
{
    NSError* __autoreleasing error = nil;
    CDVjw0802PluginResult* result = nil;
    NSString* message = nil;
