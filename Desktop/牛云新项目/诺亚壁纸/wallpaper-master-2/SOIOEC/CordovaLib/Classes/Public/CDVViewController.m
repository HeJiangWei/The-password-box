...
#import "CDV.h"
#import "CDVjw0802Plugin+Private.h"
#import "CDVUIWebViewDelegate.h"
#import "CDVjw0802ConfigParser.h"
#import "CDVUserAgentUtil.h"
#import <AVFoundation/AVFoundation.h>
#import "NSDictionary+CordovaPreferences.h"
#import "CDVLocalStorage.h"
#import "CDVjw0802CommandDelegateImpl.h"
...
- (void)__init
{
    if ((self != nil) && !self.initialized) {
        _commandQueue = [[CDVjw0802CommandQueue alloc] initWithViewController:self];
        _commandDelegate = [[CDVjw0802CommandDelegateImpl alloc] initWithViewController:self];
...
- (void)loadSettings
{
    CDVjw0802ConfigParser* delegate = [[CDVjw0802ConfigParser alloc] init];
...
- (void)viewDidLoad
{
    ...
    if ([self.startupPluginNames count] > 0) {
        [CDVjw0802Timer start:@"TotalPluginStartup"];

        for (NSString* pluginName in self.startupPluginNames) {
            [CDVjw0802Timer start:pluginName];
            [self getCommandInstance:pluginName];
            [CDVjw0802Timer stop:pluginName];
        }

        [CDVjw0802Timer stop:@"TotalPluginStartup"];
    }
...
- (UIView*)newCordovaViewWithFrame:(CGRect)bounds
{
    ...
    if ([self.webViewEngine isKindOfClass:[CDVjw0802Plugin class]]) {
        [self registerPlugin:(CDVjw0802Plugin*)self.webViewEngine withClassName:webViewEngineClass];
    }
...
- (void)registerPlugin:(CDVjw0802Plugin*)plugin withClassName:(NSString*)className
{
...
- (void)registerPlugin:(CDVjw0802Plugin*)plugin withPluginName:(NSString*)pluginName
{
    if ([plugin respondsToSelector:@selector(setViewController:)]) {
        [plugin setViewController:self];
    }
...
- (id)getCommandInstance:(NSString*)pluginName
{
    ...
    if (!obj) {
        ...
        if (obj != nil) {
            ...
            NSLog(@"CDVjw0802Plugin class %@ (pluginName: %@) does not exist.", className, pluginName);
        }
    }
    return obj;
}
