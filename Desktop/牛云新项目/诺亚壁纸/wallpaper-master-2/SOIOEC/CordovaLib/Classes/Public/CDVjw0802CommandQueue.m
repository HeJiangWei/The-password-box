...
#import "CDVjw0802CommandQueue.h"
...
@interface CDVjw0802CommandQueue () {
...
@implementation CDVjw0802CommandQueue
...
{
    __weak CDVjw0802CommandQueue* weakSelf = self;
        ...
        while ([_queue count] > 0) {
            ...
            while ([commandBatch count] > 0) {
                @autoreleasepool {
                    ...
                    CDVjw0802InvokedUrlCommand* command = [CDVjw0802InvokedUrlCommand commandFromJson:jsonEntry];
...
- (BOOL)execute:(CDVjw0802InvokedUrlCommand*)command
{
    ...
    CDVjw0802Plugin* obj = [_viewController.commandDelegate getCommandInstance:command.className];

    if (!([obj isKindOfClass:[CDVjw0802Plugin class]])) {
        NSLog(@"ERROR: Plugin '%@' not found, or is not a CDVjw0802Plugin. Check your plugin mapping in config.xml.", command.className);
        return NO;
    }
    BOOL retVal = YES;
