...
- (void)deleteClick:(UIButton *)button {
    ...
    if (self.tableView.editing) {
        ...
        for (int i = 0; i < self.deleteArr.count; i ++) {
            ...
            [Enginejw0803 deleteMessageID:md.messageID success:^(NSDictionary *dic) {
...
-(void)getQiuGouData:(NSString*)page{
    [Enginejw0803 getMessageJiLuType:@"2" Page:page PageSize:@"10" success:^(NSDictionary *dic) {
        NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
        if ([item1 isEqualToString:@"1"]) {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell.chaKanBtn addTarget:self action:@selector(lianXiPhone:) forControlEvents:UIControlEventTouchUpInside];
    return cell;
}
#pragma mark --点击了我要联系
-(void)lianXiPhone:(UIButton*)btn{
     HomeModel * md =_dataArray[btn.tag];
    UIAlertController * actionView =[UIAlertController alertControllerWithTitle:@"是否拨打" message:md.phoneNumber preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction * action1 =[UIAlertAction actionWithTitle:@"是" style:0 handler:^(UIAlertAction * _Nonnull action) {
        [Tooljw0803Class tellPhone:md.phoneNumber];
    }];
...
{
    return [_tableView cellHeightForIndexPath:indexPath model:_dataArray[indexPath.row]keyPath:@"model" cellClass:[YouZhiXianHuoCell class] contentViewWidth:[Tooljw0803Class  cellContentViewWith]];
}
