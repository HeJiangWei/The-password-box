...
{
    return [_tableView cellHeightForIndexPath:indexPath model:_tableArray[indexPath.row]keyPath:@"md" cellClass:[YouZhiXianHuoCell class] contentViewWidth:[Tooljw0803Class  cellContentViewWith]];
...
-(UIView*)CreatHeadView:(NSString*)chanpinID{
    ...
    [Enginejw0803 isShouCangGuoMaMessageID:strID success:^(NSDictionary *dic) {
        NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
        if ([item1 isEqualToString:@"1"]) {
...
//        .heightIs(20);
    ...
    if (_tagg==1 || _tagg==3) {
        /*
         ...
         tagg==2,最新采购详情
         */
        [Enginejw0803 ChanPinDianPuXiangQingDianPuID:_dianPuID ChanPinID:chanpinID success:^(NSDictionary *dic) {
            NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
            if ([item1 isEqualToString:@"1"]) {
                //item4产品详情  item5猜你喜欢 item3公司资料 item6上一条 item7下一条
                NSDictionary * dic4 =[dic objectForKey:@"Item4"];
        ...
        //最新采购详情
        [Enginejw0803 zuiXinCaiGouXiangQingMessageID:_messageID success:^(NSDictionary *dic) {
            NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
            if ([item1 isEqualToString:@"1"]) {
                NSDictionary * itemDicr=[dic objectForKey:@"Item2"];
                XiangQingModel * md =[[XiangQingModel alloc]initWithZuiXinCiGouXiangXiDic:itemDicr];
...
-(void)shouCang:(UIButton*)btn{
    ...
    //if (_tagg==2) {
        //代表最新采购收藏
        //Type 1.浏览 2.收藏 3.关注
        [Enginejw0803 addMessageJiLuMessageID:_messageID Type:@"2" success:^(NSDictionary *dic) {
                NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
                if ([item1 isEqualToString:@"1"]) {
                    btn.selected=YES;
                    [LCProgressHUD showMessage:[dic objectForKey:@"Item2"]];
