...
- (void)viewDidLoad {
    ...
    _bgScrollview=[[UIScrollView alloc]initWithFrame:CGRectMake(0, 64+50, ScreenWidth, ScreenHeight-64-44)];
...
-(void)CreatNameArray{
    ...
    [_bgScrollview addSubview:_view3];
}
...
-(void)CreatTableView{
    ...
    _tableView.frame=CGRectMake(0, 0, ScreenWidth, 6*50+5+5);
...
-(void)CreatView3{
    ...
    [_bgScrollview sd_addSubviews:@[_publicBtn]];
    _publicBtn.sd_layout
    .leftSpaceToView(_bgScrollview,30)
    .rightSpaceToView(_bgScrollview,30)
...
-(void)fabu:(UIButton*)btn{
   ...
   //所在地 _diquText _cityCode
    ...
    [Enginejw0803 qiYeKuaiSuPublicTitleStr:cell0.textField.text Count:cell1.textField.text PriceStr:cell2.textField.text PhoneNumber:cell3.textField.text HangYeID:[Tooljw0803Class isString:_hangYeCode] DiQuCode:[Tooljw0803Class isString:_cityCode] imageUrlStr:[Tooljw0803Class isString:_urlStr] success:^(NSDictionary *dic) {
        NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
        if ([item1 isEqualToString:@"1"]) {
            [LCProgressHUD showMessage:[dic objectForKey:@"Item2"]];
