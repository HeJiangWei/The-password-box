...
-(void)jieXiData{
    ...
    [Enginejw0803 huoQuDianPuFirstMessageID:_messageID success:^(NSDictionary *dic) {
...
//店铺id传递的是
    [Enginejw0803 huoQuDianPuFirstMessageID:_messageID success:^(NSDictionary *dic) {
        ...
        if ([item1 isEqualToString:@"1"]) {
            ...
            //头像
            [headImage setImageWithURL:[NSURL URLWithString:[Tooljw0803Class isString:[NSString stringWithFormat:@"%@%@",IMAGE_TITLE,[dicc objectForKey:@"M_HeadImg"]]]] placeholderImage:[UIImage imageNamed:@"my_photo"]];
            //公司名字
             comLabel.text=[Tooljw0803Class isString:[dicc objectForKey:@"M_CompanyName"]];
            //地区
            address.text=[Tooljw0803Class isString:[dicc objectForKey:@"M_Address"]];
            //收藏次数//字段不确定
            shouCang.text=[Tooljw0803Class isString:[NSString stringWithFormat:@"收藏次数  %@",[dicc objectForKey:@"M_Category"]]];
            //400电话
            [phoneBtn setTitle:[Tooljw0803Class isString:[NSString stringWithFormat:@"%@",[dicc objectForKey:@"E_Number"]]] forState:0];
            //M_Id 是店铺ID
        }else{
            [LCProgressHUD showMessage:[dic objectForKey:@"Item2"]];
        }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        return [_tableView cellHeightForIndexPath:indexPath model:_dataArray1[indexPath.row]keyPath:@"model" cellClass:[JinDianChaKanCell class] contentViewWidth:[Tooljw0803Class  cellContentViewWith]];;
  
    }else if (indexPath.section==1){
        return [_tableView cellHeightForIndexPath:indexPath model:_dataArray2[indexPath.row]keyPath:@"model" cellClass:[JinDianChaKanCell class] contentViewWidth:[Tooljw0803Class  cellContentViewWith]];;

    }else{
        return [_tableView cellHeightForIndexPath:indexPath model:_dataArray3[indexPath.row]keyPath:@"model" cellClass:[JinDianChaKanCell class] contentViewWidth:[Tooljw0803Class  cellContentViewWith]];;
