...
-(void)CreatView1{
   ...
   //编号
    ...
    [Enginejw0803 genJuJingJiRenID:_peopleID success:^(NSDictionary *dic) {
   ...
   // [wangji addTarget:self action:@selector(wangji) forControlEvents:UIControlEventTouchUpInside];
    ...
    int d =(ScreenWidth-102*3)/4;
        for (int i =0; i<6; i++) {
            ...
            btn.sd_layout
            .leftSpaceToView(_view2,d+(204/2+d)*(i%3))
    ...
    //合计
    hejiLab =[UILabel new];
    hejiLab.text=@"合计  1000元";
    hejiLab.textColor=[UIColor redColor];
    hejiLab.font=[UIFont systemFontOfSize:20];
    hejiLab.attributedText=[Tooljw0803Class attrStrFrom:hejiLab.text intFond:16 Color:[UIColor blackColor] numberStr:@"合计"];
...
-(void)btnClick:(UIButton*)btn{
   ...
   static int jlu=0;
    if (_lastBtn.tag == btn.tag) {
        ...
        if (_lastBtn.selected==NO) {
            hejiLab.text=[NSString stringWithFormat:@"合计   %d元",jlu-d];
          NSLog(@"减去%@",hejiLab.text);
        }else{
            hejiLab.text=[NSString stringWithFormat:@"合计   %d元",1000+d];
            jlu=1000+d;
            NSLog(@"加上%@",hejiLab.text);
        }
