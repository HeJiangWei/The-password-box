...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row==2) {
            //我要认证
            MyRenZhengjw0803VC * vc =[MyRenZhengjw0803VC new];
            [self.navigationController pushViewController:vc animated:YES];
        }else{
            //个人资料、公司资料
            Messagejw0803VC * vc =[Messagejw0803VC new];
        ...
        UIAlertAction * action1 =[UIAlertAction actionWithTitle:@"确认" style:0 handler:^(UIAlertAction * _Nonnull action) {
            //删除Login.plist文件
            [Tooljw0803Class deleagtePlistName:@"Login"];
            //移除token
...
-(void)saveImage:(NSString*)url{
     ...
     */
    [Enginejw0803 saveImageType:@"0" urlStr:url success:^(NSDictionary *dic) {
        NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
        if ([item1 isEqualToString:@"1"]) {
            [LCProgressHUD showMessage:[dic objectForKey:@"Item2"]];
        }else{
