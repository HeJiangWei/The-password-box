...
#import "MyRenZhengjw0803VC.h"

@interface MyRenZhengjw0803VC ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@implementation MyRenZhengjw0803VC
...
-(void)CreatView1{
    ...
    if (_image1) {
        ...
        //获取身份证图片
        [Enginejw0803 huoQuImageWithType:@"1" success:^(NSDictionary *dic) {
    ...
    if (_image3) {
        ...
        [Enginejw0803 huoQuImageWithType:@"3" success:^(NSDictionary *dic) {
            NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
            if ([item1 isEqualToString:@"1"]) {
                NSArray * imageHead =[dic objectForKey:@"Item3"];
                for (NSDictionary * dicc  in imageHead) {
...
-(void)CreatView2{
    ...
    if (_image2) {
        ...
        //获取身份证图片
        [Enginejw0803 huoQuImageWithType:@"4" success:^(NSDictionary *dic) {
            NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
            if ([item1 isEqualToString:@"1"]) {
                NSArray * imageHead =[dic objectForKey:@"Item3"];
                for (NSDictionary * dicc  in imageHead) {
...
-(void)shangChuanImage:(UIImage*)image tagg:(NSString*)tag{
    ...
    //type:1.产品 2新闻 3.用户
    [Enginejw0803 ShangChuanImageData:imgData Type:@"3" success:^(NSDictionary *dic) {
        NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
        if ([item1 isEqualToString:@"1"]) {
...
-(void)saveImage:(NSString*)url tagg:(NSString*)tag{
     ...
     */
    [Enginejw0803 saveImageType:tag urlStr:url success:^(NSDictionary *dic) {
        NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
        if ([item1 isEqualToString:@"1"]) {
            [LCProgressHUD showMessage:[dic objectForKey:@"Item2"]];
        }else{
