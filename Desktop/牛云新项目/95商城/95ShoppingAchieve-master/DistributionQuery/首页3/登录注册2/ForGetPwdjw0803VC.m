...
@property(nonatomic,strong)UITextField *pwdText;//请输入新密码
...
@implementation ForGetPwdjw0803VC
...
-(void)CreatLoginBtn{
    ...
    [_commentBtn setTitle:@"提交" forState:0];
...
-(void)tijiaoBtn{
    ...
    [Enginejw0803 xiuGaiMiMaPhone:_phoneText.text Code:_codeText.text Password:_pwdText.text success:^(NSDictionary *dic) {
...
-(void)codeBtnClick:(UIButton*)sender{
    ...
    [Enginejw0803 getCodePhone:_phoneText.text typeStr:@"3" success:^(NSDictionary *dic) {
        NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
        if ([item1 isEqualToString:@"1"]) {
            [LCProgressHUD hide];
            //实现倒计时
