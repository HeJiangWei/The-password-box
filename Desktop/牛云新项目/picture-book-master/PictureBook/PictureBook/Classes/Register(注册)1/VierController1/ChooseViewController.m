...
- (void)setConstraint{
    ...
    [self.sureBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.allBtn.bottom).offset(20);
        make.centerX.equalTo(self.view);
    }];
}
- (void)skip{
    ...
    [YYHttp1Tool get:@"http://app.52kb.cn:666/get_shut_count" params:dic success:^(id responseObj) {
...
- (void)completeRegister:(NSString *)organization{
    if (organization.length>0) {
        ...
        [YYHttp1Tool get:@"http://app.52kb.cn:666/set_register_info" params:dic success:^(id responseObj) {
            NSString *string = responseObj[@"msg"];
            NSLog(@"注册========%@",string);
        } failure:^(NSError *error) {
            NSLog(@"error");
