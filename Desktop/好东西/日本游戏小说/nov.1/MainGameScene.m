...
#import "Title1Layer.h"
#import "LoadScene.h"
#import "SaveScene.h"
#import "Ending1Layer.h"
...
- (void)doInstruct:(NSMutableArray *)instruct spriteSize:(CGSize) size{
    for (int i=0; i<[instruct count]; i++){
        ...
        if([[dictionary objectForKey:@"instruct_name"] isEqualToString:@"# BG"]){
            ...
            [super onEnterTransitionDidFinish];
            [[CCDirector sharedDirector] replaceScene:[CCTransitionFade transitionWithDuration:5.f scene:[Ending1Layer scene] withColor:ccWHITE]];
...
-(BOOL) ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    if(location.x > self.menu_image.position.x-(self.menu_image.contentSize.width/2)-15.f&&
        ...
        // メニューに移動
        CCScene   *scene     = [Menu1Scene scene];
        Menu1Scene *Menu1Scene = [scene.children objectAtIndex:0];
        ...
        width                    = 267.f;
        height                   = 205.f;
        Menu1Scene.screen_capture = [self croppingImage:image imageRect:CGRectMake((image.size.width/2.f)-(width/2.f), 0.f, width, height)];
        Menu1Scene.save_text      = self.message_text;
