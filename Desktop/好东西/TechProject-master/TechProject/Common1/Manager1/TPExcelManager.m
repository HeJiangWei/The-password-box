...
- (NSArray *)readClientInfo:(DHxlsReader *)reader{
    ...
    while (YES) {
        ...
        if (cell.str) {
            TPClient1Model *model = [self readSingleClient:reader row:row typeArr:typeArr];
...
- (TPClient1Model *)readSingleClient:(DHxlsReader *)reader row:(NSInteger)row typeArr:(NSArray *)typeArr{
    TPClient1Model *client = [TPClient1Model new];
...
- (BOOL)alreadyContainProject:(NSString *)name{
    ...
    for (TPProject1Model *model in pArr) {
        if ([model.name isEqualToString:name]) {
            return YES;
        }
    }
...
- (NSArray *)readProjectInfo:(DHxlsReader *)reader regionArr:(NSArray *)regionArr{
    ...
    TPProject1Model *project = [TPProject1Model new];
    while (YES) {
