...
@interface ShowIMGCollection1Cell ()
...
@implementation ShowIMGCollection1Cell
...
//    self.showIMGBtn.contentMode = UIViewContentModeScaleAspectFit;
//    [self.contentView addSubview:self.showIMGBtn];
    
    self.showIMGView = [[UIImageView alloc]initWithFrame:CGRectMake(3, 3, WIDTH_PIC-6, WIDTH_PIC-6)];
    self.showIMGView.contentMode = UIViewContentModeCenter;
    self.showIMGView.clipsToBounds = YES;
    [self.contentView addSubview:self.showIMGView];
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(showBigIMGClick)];
    ...
    [self.selectedBtn setImage:[UIImage imageNamed:@"moren"] forState:UIControlStateNormal];
    [self.selectedBtn setImage:[UIImage imageNamed:@"xuanzhong"] forState:UIControlStateSelected];
...
//        self.selectedBtn.selected = NO;
        self.selectedBlock(NO,self.selectedBtn);
    }else{
//        self.selectedBtn.selected = YES;
        ...
        self.selectedBlock(YES,self.selectedBtn);
    }
}
...
- (void)configWithModel:(ShowIMGModel *)model{
    ...
    initialRequestOptions.version = PHImageRequestOptionsVersionCurrent;
