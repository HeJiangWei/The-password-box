...
#define RulerLong       20
#define RulerShort      15
#define TrangleWidth    10
#define CollectionHeight 30
...
-(void)drawRect:(CGRect)rect{
    ...
    for (int i = 0; i <= _betweenNumber; i ++){
        ...
        if (i%_betweenNumber == 0){
            ...
            NSDictionary *attribute = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:[UIColor colorWithHexString:@"999999"]};
            ...
            [num drawInRect:CGRectMake(startX+lineCenterX*i-width/2, longLineY+10, width, 0) withAttributes:attribute];
...
-(void)drawRect:(CGRect)rect{
    ...
    CGFloat width = [num boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:0 attributes:attribute context:nil].size.width;
    [num drawInRect:CGRectMake(rect.size.width-width/2, longLineY+10, width, 9) withAttributes:attribute];
...
-(void)drawRect:(CGRect)rect{
    ...
    NSDictionary *attribute = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:[UIColor colorWithHexString:@"999999"]};
    CGFloat width = [num boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:0 attributes:attribute context:nil].size.width;
    [num drawInRect:CGRectMake(0-width/2, longLineY+10, width, 9) withAttributes:attribute];
...
-(instancetype)initWithFrame:(CGRect)frame theMinValue:(float)minValue theMaxValue:(float)maxValue theStep:(float)step theUnit:(NSString *)unit theNum:(NSInteger)betweenNum{
    ...
    if (self) {
        ...
        _bgColor    = [UIColor colorWithHexString:@"f8f8f8"];
...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        _collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_valueTF.frame)+5, self.bounds.size.width, CollectionHeight) collectionViewLayout:flowLayout];
...
-(UITextField *)valueTF{
    if (!_valueTF) {
        _valueTF                          = [[UITextField alloc]initWithFrame:CGRectMake(self.bounds.size.width/2 - 20, -5,40, 9)];
...
-(UILabel *)unitLab{
    if (!_unitLab) {
        _unitLab = [[UILabel alloc]initWithFrame:CGRectMake( self.bounds.size.width/2+20 , 10, 40, 8)];
...
//            rulerView.backgroundColor   =  [UIColor whiteColor];
//        }else{
            rulerView.backgroundColor   =  [UIColor colorWithHexString:@"f8f8f8"];
//        }
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (_isDraging && [self.delegate respondsToSelector:@selector(dyScrollRulerView:valueChange:)]) {
