...
- (void)setUp {
    ...
    _cropAreaCornerLineColor = [UIColor redColor];
...
- (void)handleCornerPan: (UIPanGestureRecognizer *)panGesture {
    ...
    if(panView == _topLeftCorner) {
        ...
        approachAspectRatio = (WIDTH(self) - MAXX(panView) + self.cornerMargin) / (MINY(panView) + self.cornerMargin);
    }
    else if(panView == _bottomLeftCorner) {
        approachAspectRatio = (MINX(panView) + self.cornerMargin) / (HEIGHT(self) - MAXY(panView) + self.cornerMargin);
    }
    else if(panView == _bottomRightCorner) {
        approachAspectRatio = (WIDTH(self) - MAXX(panView) + self.cornerMargin) /(HEIGHT(self) - MAXY(panView) + self.cornerMargin);
    }
...
- (void)resetImageView {
    ...
    if(_imageAspectRatio > selfAspectRatio) {
        ...
        self.frame = CGRectMake(0, _paddingTopBottom, WIDTH(self), floor(WIDTH(self) / _imageAspectRatio));
    }
    else {
        _paddingTopBottom = 0;
        _paddingLeftRight = floor((WIDTH(self) - HEIGHT(self) * _imageAspectRatio) / 2.0);
        self.frame = CGRectMake(_paddingLeftRight, 0, floor(HEIGHT(self) * _imageAspectRatio), HEIGHT(self));
    }
...
- (void)resetCropAreaByAspectRatio {
    ...
    if(_cropAspectRatio == 0) {
        width = (WIDTH(self) - 2 * tmpCornerMargin) * _initialScaleFactor;
        height = (HEIGHT(self) - 2 * tmpCornerMargin) * _initialScaleFactor;
        if(_showMidLines) {
            [self createMidLines];
            [self resetMidLines];
        }
...
//        }
    ...
    _cropAreaView.frame = CGRectMake((WIDTH(self) - width) / 2.0, (HEIGHT(self) - height) / 2.0, width, height);
    [self resetCornersOnCropAreaFrameChanged];
    [self resetCropTransparentArea];
...
- (UIImage *)currentCroppedImage {
    ...
    return  [Tools imageFromView:self atFrame:CGRectMake((MINX(_cropAreaView) + _cropAreaBorderLineWidth) / scaleFactor, (MINY(_cropAreaView) + _cropAreaBorderLineWidth) / scaleFactor, (WIDTH(_cropAreaView) - 2 * _cropAreaBorderLineWidth) / scaleFactor, (HEIGHT(_cropAreaView) - 2 * _cropAreaBorderLineWidth) / scaleFactor)];
