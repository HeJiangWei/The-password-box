...
@interface JW0905NJKWebViewProgressView : UIView
@property (nonatomic) float progress;

@property (nonatomic) UIView *progressBarView;
@property (nonatomic) NSTimeInterval JW0905barAnimationDuration; // default 0.1
@property (nonatomic) NSTimeInterval JW0905fadeAnimationDuration; // default 0.27
@property (nonatomic) NSTimeInterval JW0905fadeOutDelay; // default 0.1

- (void)setProgress:(float)progress animated:(BOOL)animated;
@end

@implementation JW0905NJKWebViewProgressView
...
- (void)setup {
    ...
    _JW0905barAnimationDuration = 0.27f;
    _JW0905fadeAnimationDuration = 0.27f;
    _JW0905fadeOutDelay = 0.1f;
}
...
- (void)setProgress:(float)progress animated:(BOOL)animated {
    ...
    [UIView animateWithDuration:(isGrowing && animated) ? _JW0905barAnimationDuration : 0.0 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
    ...
    if (progress >= 1.0) {
        [UIView animateWithDuration:animated ? _JW0905fadeAnimationDuration : 0.0 delay:_JW0905fadeOutDelay options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        [UIView animateWithDuration:animated ? _JW0905fadeAnimationDuration : 0.0 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
...
@interface JW0905BaseWebController ()
...
@property (nonatomic, strong) JW0905NJKWebViewProgressView *progressView;
...
@end
@implementation JW0905BaseWebController
+ (instancetype)jw0704JW0905KDManager:(NSString *)urlString {
    return [self jw0704JW0905KDManager:urlString shareTitle:nil shareUrl:nil];
}
+ (instancetype)jw0704JW0905KDManager:(NSString *)urlString shareTitle:(NSString *)shareTitle shareUrl:(NSString *)shareUrl {
    JW0905BaseWebController *webView = [[self alloc] init];
...
- (void)webView:(WKWebView *)webView runJavaScriptAlertPanelWithMessage:(NSString *)message initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(void))completionHandler {
    [JW0905KDAlertView JW0905showTitle:nil message:message buttons:@[@"确定"] completion:^(NSUInteger index, NSString *buttonTitles) {
        completionHandler();
    }];
}
...
- (void)didClickMenu:(id)sender {
    NSLog(@"didClickMenu");
    [KDActionSheet JW0905showTitle:nil message:nil buttons:@[@"分享", @"清除缓存"] completion:^(NSUInteger index, NSString *buttonTitles) {
...
- (JW0905NJKWebViewProgressView *)progressView {
    if (!_progressView) {
        CGFloat progressBarHeight = 2.f;
        CGRect navigationBarBounds = self.navigationController.navigationBar.bounds;
        CGRect barFrame = CGRectMake(0, navigationBarBounds.size.height - progressBarHeight, navigationBarBounds.size.width, progressBarHeight);
        _progressView = [[JW0905NJKWebViewProgressView alloc] initWithFrame:barFrame];
